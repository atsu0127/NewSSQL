		...
		function get_SSQL_query($t){
			...
			$query1 = "
				GENERATE ResponsiveHTML {
				       [null((desc1)s.num_of_models)!
				        s.num_of_models||'モデルベース'@{height=40, font-size=20, color='#6699FF', padding=2, align='center', valign='bottom'}!
				       [ {{glink((asc2)s.id||':  '||s.d@{height=30, font-size=13, padding=2, align='center'}, './r_details.html', s.id)!
				          image(s.name||'/".$t.".png".d()."', './r')@{align='center', height=250}
				         }@{padding=8, align='center'}}@{sm='1/2', md='1/3'}
				       ], !
				       ' '@{height=40}
				       ]!
			        } FROM stock_model s;
			...
			$query1 = "
				GENERATE ResponsiveHTML 
				       [s.num_of_models, 
				       		[image(s.name||'/".$t.".png".d()."', './r')@{align='center'}, {s.id, s.d}]!
				       ]!@{responsive}
			        FROM stock_model s;
			";
			//@{responsive}		TEST2
