...
(function () {
  ...
  function sendResourcePerfEntry(entry) {
    try {
        ...
        try {
          ...
          RAT.addCustomEvent({
            ...
            pData: {
              ...
              head_time: (new Date().getTime() - startTime)
            }
          });
        } catch (e) {}
      });
  ...
  function sendResourcePerfEntries(entries) {
    RAT.addCustomEvent({
      options: ['url','ua'],
      eventType: 'perf',
      pData: {
  ...
  function sendResourcePerfData() {
    ...
    if (res && res.length) {
      ...
      // Get a sample metric for each target domain
      ...
      if (parameters.acc === 1228) {
        ...
        for (i = 0; i < metrics.length; ++i) {
          var entry = metrics[i];
          sendResourcePerfEntry(entry);
        }
      } else {
        sendResourcePerfEntries(metrics);
      }
    }
  }
  if ((parameters.acc === 1 || parameters.acc === 1228) && parameters.aid === 1) {
